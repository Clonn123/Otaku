                                ...
                                public void onComplete(@NonNull Task<Void> task) {
                                    if(task.isSuccessful()){
                                        ...
                                        user.sendEmailVerification().addOnCompleteListener(new OnCompleteListener<Void>() {
                                            ...
                                            public void onComplete(@NonNull Task<Void> task) {
                                                if (task.isSuccessful()){
                                                    Log.d("Email", "Email отпр.");
                                                    Toast.makeText(MainActivity.this, "Подтверждение email отпр.", Toast.LENGTH_SHORT).show();
                                                }
                                            }
                                        });
                                    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        forgotpass.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (userEmail.isEmpty()) {
                    email.setError("E-mail требуется");
                    email.requestFocus();
                    return;
                } else {
                    mAuth.sendPasswordResetEmail(userEmail).addOnCompleteListener(new OnCompleteListener<Void>() {
                        ...
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()) {
                                Log.d("Сброс пароля", "Email отпр.");
                                Toast.makeText(MainActivity.this, "Сброс пароля e-mail отпр!", Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
                }
        ...
        bLogin.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View vLogin) {
                ...
                if (password.isEmpty()){
                    pass.setError("Пароль требуется");
                    pass.requestFocus();
                    return;
                }
                else{
                    mAuth.signInWithEmailAndPassword(userEmail, password).addOnCompleteListener(MainActivity.this, new OnCompleteListener<AuthResult>() {
                        ...
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            if(task.isSuccessful()){
                                ...
                                Log.w("123", "Вход по почте: неудачно", task.getException());
                                Toast.makeText(MainActivity.this, "Аутентификация неудачна!", Toast.LENGTH_LONG).show();
                                updateUI(null);
                            }
                        }
                    });
